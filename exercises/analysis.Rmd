...
```{r}
adjcrudeOR <- glm('dili ~ log10base_alt + agegroup + sex + log10base_alp + log10base_bili', family=binomial(link='logit'), data=filter(df_tidy, dili=='T1' | dili=='C')) 
```
...
```{r}
adjcrudeORdf <- as.data.frame(summary(adjcrudeOR)$coefficient)
head(adjcrudeORdf)
```
